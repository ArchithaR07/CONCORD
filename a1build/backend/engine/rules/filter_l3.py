...
def generate_candidate_pairs(obligations, similarity_threshold=None, max_per_bucket=None):
    ...
    obligations: list of dicts matching obligation.schema.json (must include
                 'id', 'topic', 'embedding', "policy_file").
    Returns: list of candidate pair dicts.
    """
    similarity_threshold = similarity_threshold or config.EMBEDDING_CANDIDATE_THRESHOLD
    ...
    for topic, member_ids in by_topic.items():
        ...
        for id1, id2 in combo_iter:
            ...
            if obligations[id_to_idx[id1]]["policy_file"] == obligations[id_to_idx[id2]]["policy_file"]:
                continue
    ...
    # n = len(obligations)
    # doc_ids = [o["policy_file"] for o in obligations]
