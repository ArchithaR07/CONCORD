...
def _policy_ref(obl):
    ...
    return f"{obl['policy']} \u00a7{section}" if section else obl["policy"]
...
def resolve_precedence(obl_a, obl_b):
    ...
    rank_a, rank_b = _authority_rank(obl_a["policy"]), _authority_rank(obl_b["policy"])
    if rank_a != rank_b:
        winner_name = name_a if rank_a > rank_b else name_b
        chain.append(f"Authority tiebreaker: {obl_a['policy']} (rank {rank_a}) vs {obl_b['policy']} (rank {rank_b}).")
    ...
    chain.append(f"Authority tiebreaker also ties ({obl_a['policy']} and {obl_b['policy']} share rank {rank_a}).")
...
def build_resolved_findings(rule_verdicts, trust_scores_by_pair, obligations_by_id):
    ...
    for v in rule_verdicts:
        ...
        findings.append({
            ...
            "verdict_source": trust["verdict_source"],
